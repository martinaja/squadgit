...
const loginStorage = localStorage.getItem('login') ? JSON.parse(localStorage.getItem('login')) : '';

const LoginContext = ({ children }) => {
	// children es el componente que se esta renderizando
	...
	const handleLogin = e => {
		// Funcion que permite hacer el login
		...
		if (
			...
			localStorage.setItem(
				// Guardo el email y el password en el localStorage
				'login',
	...
	return (
		<loginContext.Provider // Creo el contexto del login
			value={{
				// Seteo los valores del contexto
				email: email || loginStorage.email, // Email
